...
namespace iTec_uwp
{
    ...
    /// 可以在本身使用或巡覽至框架內的空白頁面。
    /// </summary>
    public sealed partial class Sensor1_Page : Page
    {
        ...
        {
            txtPowerValue.Text = string.Format("{0}", Convert.ToInt16(GV.ITEC_HMI.Power));
            rpcPower.Value = GV.ITEC_HMI.Power;
            txtCadenceValue.Text = string.Format("{0}", Convert.ToInt16(GV.ITEC_HMI.Cadence));
            rpcCadence.Value = GV.ITEC_HMI.Cadence;
